...
use Phabriks\Phit\Project\JsonFile as JsonFile;
use Phabriks\Phit\Config as BaseConfig;
...
class Config extends BaseConfig
{
    ...
    public function __construct(JsonFile $configFile)
    {
        if ($configFile->exists()) {
            if ($configFile->validateSchema(JsonFile::LAX_SCHEMA)) {
                $configData = $configFile->read();
            } else {
                throw new \RuntimeException('Project configuration file not valid' . $configFile->getPath());
            }
        } else {
            throw new \RuntimeException('No project configuration file ' . $configFile->getPath());
        }
